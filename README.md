...
| **Waveshare ESP32-S3-Touch-LCD-1.28** (240×240 round) | `waveshare-esp32-s3-touch-lcd-1` | GPIO **0** | TFT backlight GPIO **2** |

DOIT / WROOM / WROVER builds use the same BLE/camera logic as the C6 demo. **Lily Pi** and **Waveshare S3 LCD** add an LVGL status screen (BLE/camera, GPS fix, recording); GPS UART remains **16/17** @ 115200 on the SH1.0 / devkit header for HC-05 NMEA. Without an LC76G module you can still connect and control the camera; GPS/dashboard features need a GNSS module (or C6 + LC76G wiring on GPIO4/5 for C6 only).
