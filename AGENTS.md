...
- Waveshare ESP32-S3-Touch-LCD-1.28: PlatformIO env `waveshare-esp32-s3-touch-lcd-1` (`CONFIG_WAVESHARE_ESP32_S3_TOUCH_LCD_128`); GC9A01 240×240 SPI (MOSI 11, SCLK 10, CS 9, DC 8, RST 14, BL 2); GPS UART2 on SH1.0 GPIO16/17; USB serial GPIO43/44. CH343 exposes two macOS ports — use `/dev/cu.wchusbserial*` for upload, not `usbmodem*` (checksum errors on wrong port).
